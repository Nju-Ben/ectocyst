...
#merge_data = merge_data[(merge_data.index >= '2011-01-01')] #after_crisis
...
#for k in range(3):
    backtest_return = []
    #for i in range(len(merge_data)-look_back_days):
    for i in range(len(merge_data)-look_back_days):
    #for i in range(len(outsample_data)-1):
        ...
        backtest_return.append(w_p.dot(np.array(merge_data.iloc[look_back_days+i,0:12])))
        j = j+1
        #print(portfolio)
...
m = len(merge_data)
backtest_data = merge_data.iloc[look_back_days:m,:]
...
backtest_data['portfolio_return_b_1.5'] =  br[0]
backtest_data['portfolio_return_b_0.5'] =  br[1]
backtest_data['portfolio_return_b_0'] =  br[2]
backtest_data['portfolio_return_b_-0.5'] =  br[3]
backtest_data['portfolio_return_b_-1.5'] =  br[4]
...
def cal_indicator(dataframe):
    ...
    #md = MaxDrawdown(np.array(backtest_nv[column]))
